...
{
	global $con;
	$sql="DELETE FROM `vs_product` WHERE `product_id`='".mysqli_real_escape_string($con,$ProjectID)."' ";
...
function changeorderstatusbydid($order_id,$status,$cart_id,$redir)
{
	...
	$sql="UPDATE `vs_order` SET `current_status`='".mysqli_real_escape_string($con,$status)."' WHERE `order_id`='".mysqli_real_escape_string($con,$order_id)."' ";
	if($con->query($sql))
	{

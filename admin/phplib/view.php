...
{
	global $con;
	$sql="SELECT `product_id`, `product_name`, `image`, `price`, `selling`, `special`,`SKU` FROM `vs_product` ";
	$result = $con->query($sql);
...
function ViewProductByID($id)
{
	...
	//sql
	$sql="SELECT * FROM `vs_product` WHERE `product_id`='".mysqli_real_escape_string($con,$id)."' ";
	$result = $con->query($sql);
...
function viewpendingorder()
{
	...
	//sql
	$sql="SELECT `order_id`,`cart_id`,`order_by`,`totalamount`,`deliverycharge`,`mode`,`date`,`time` FROM `vs_order` WHERE `payment`='1' AND `current_status`='Pending' ORDER BY `order_id` DESC ";
	$result = $con->query($sql);
...
function viewproductitembycart($cart_id)
{
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$sql1="SELECT product_id,product_name,image,selling,SKU FROM `vs_product` WHERE `product_id`='".mysqli_real_escape_string($con,$row['product_id'])."'";
			$result1= $con->query($sql1);
			while($row1 = $result1->fetch_assoc())
			{
				echo "<tr>
...
{
	global $con;
	$sql="SELECT * FROM `vs_order` WHERE `order_id`='".mysqli_real_escape_string($con,$order_id)."' ";
	$result=$con->query($sql);
	if($result->num_rows>0)
	{
		while($row = $result->fetch_assoc()) {
...
{
	global $con;
	$sql="SELECT `order_id`,`cart_id`,`order_by`,`totalamount`,`deliverycharge`,`mode`,`date`,`time` FROM `vs_order` WHERE `payment`='1' ORDER BY `order_id` DESC ";
	$result = $con->query($sql);
...
function viewpendingreview()
{
	...
	if ($result->num_rows > 0) {
		while($row=$result->fetch_assoc())
		{
			$sql1="SELECT `product_name`,`image` FROM `vs_product` WHERE `product_id`='".$row['product_id']."' ";
			$result1=$con->query($sql1);

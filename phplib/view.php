...
{
	global $con;
	$sql="SELECT * FROM `vs_product` ORDER BY RAND() DESC LIMIT 20";
	$result = $con->query($sql);
...
{
	global $con;
	$sql="SELECT * FROM `vs_product` WHERE `product_id`='".$id."'  ";
	$result = $con->query($sql);
	if ($result->num_rows > 0) {
...
{
	global $con;
	$sql="SELECT * FROM `vs_product` ORDER BY RAND() LIMIT $limit";
	$result = $con->query($sql);
...
function viewproductbycategory($category_id)
{
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$sql1="SELECT * FROM vs_product WHERE product_id='".$row['product_id']."' ";
...
{
	global $con;
	$sql="SELECT * FROM `vs_product` ORDER BY RAND() LIMIT 20";
	$result = $con->query($sql);
...
function viewcartitem()
{
	if(isset($_SESSION['uniqueSessionID']))
	{
		global $con;
	$total=0;
	$sql="SELECT * FROM `vs_cart` WHERE `User_ID`='".mysqli_real_escape_string($con,$_SESSION['uniqueSessionID'])."' ";
	$result=$con->query($sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc())
		{
			$sqlcommand="SELECT * FROM `vs_product` WHERE `product_id`='".mysqli_real_escape_string($con,$row['product_id'])."' ";
...
{
	global $con;
	$sql="SELECT * FROM `vs_user` WHERE `customer_email`='".mysqli_real_escape_string($con,$_SESSION['UserEmail'])."' ";
	$result=$con->query($sql);
...
{
	global $con;
	$sql="SELECT * FROM `vs_order` WHERE `payment`=1 AND `email`='".mysqli_real_escape_string($con,$_SESSION['UserEmail'])."' ORDER BY `order_id` DESC";
...
{
	global $con;
	$sql="SELECT * FROM `vs_order` WHERE `order_id`='".mysqli_real_escape_string($con,$id)."' ";
	$result=$con->query($sql);
	$row = $result->fetch_assoc();
	return $row;
}
...
{
	global $con;
	$sql="SELECT * FROM `vs_cart` WHERE `User_ID`='".mysqli_real_escape_string($con,$cart_id)."' ";
	$result = $con->query($sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$sql1="SELECT product_id,product_name,image,selling FROM `vs_product` WHERE `product_id`='".mysqli_real_escape_string($con,$row['product_id'])."'";
			$result1= $con->query($sql1);
			while($row1 = $result1->fetch_assoc())
			{
				if($status=="Delivered")
...
{
	global $con;
	$sql="SELECT * FROM `vs_productimage` WHERE `product_id`='".mysqli_real_escape_string($con,$id)."' ";
	$result = $con->query($sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo "
...
{
	global $con;
	$sql="SELECT * FROM `vs_product` ORDER BY RAND()";
	$result = $con->query($sql);
...
{
	global $con;
	$sql="SELECT * FROM `vs_product` WHERE `product_name` LIKE '%".$keyword."%' LIMIT 20";
	$result = $con->query($sql);
